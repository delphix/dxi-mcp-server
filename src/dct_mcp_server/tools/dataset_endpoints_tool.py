...
* `external` - MSS...
    ...
    if action == 'list_vdbs':
        ...
        if conf:
            return conf
        body = {k: v for k, v in {'unique_name': unique_name, 'database_name': database_name, 'repository_id': repository_id, 'environment_user_ref': environment_user_ref, 'tde_keystore_password': tde_keystore_password, 'tde_keystore_config_type': tde_keystore_config_type, 'oracle_instance_name': oracle_instance_name, 'instance_number': instance_number, 'instances': instances, 'mount_base': mount_base, 'config_params': config_params, 'cdb_id': cdb_id, 'parent_tde_keystore_path': parent_tde_keystore_path, 'parent_tde_keystore_password': parent_tde_keystore_password, 'tde_exported_keyfile_secret': tde_exported_keyfile_secret, 'tde_key_identifier': tde_key_identifier, 'parent_pdb_tde_keystore_path': parent_pdb_tde_keystore_path, 'parent_pdb_tde_keystore_password': parent_pdb_tde_keystore_password, 'target_pdb_tde_keystore_password': target_pdb_tde_keystore_password, 'crs_database_name': crs_database_name, 'recover_database': recover_database, 'file_mapping_rules': file_mapping_rules, 'enable_cdc': enable_cdc, 'recovery_model': recovery_model, 'mirroring_state': mirroring_state, 'is_incremental_v2p': is_incremental_v2p, 'backup_frequency_minutes': backup_frequency_minutes, 'rman_channels_for_incremental_backup': rman_channels_for_incremental_backup, 'rman_files_per_set_for_incremental_backup': rman_files_per_set_for_incremental_backup, 'rman_file_section_size_in_gb_for_incremental_backup': rman_file_section_size_in_gb_for_incremental_backup, 'invoke_datapatch': invoke_datapatch, 'default_data_diskgroup': default_data_diskgroup, 'redo_diskgroup': redo_diskgroup, 'rman_channels': rman_channels, 'rman_file_section_size_in_gb': rman_file_section_size_in_gb, 'snapshot_id': snapshot_id}.items() if v is not None}
        return await make_api_request('POST', endpoint, params=params, json_body=body if body else None)
        ...
        if conf:
            return conf
        body = {k: v for k, v in {'name': name, 'source_id': source_id, 'group_id': group_id, 'description': description, 'log_sync_enabled': log_sync_enabled, 'sync_policy_id': sync_policy_id, 'retention_policy_id': retention_policy_id, 'make_current_account_owner': make_current_account_owner, 'tags': tags, 'ops_pre_sync': ops_pre_sync, 'ops_post_sync': ops_post_sync, 'db_vault_username': db_vault_username, 'db_vault': db_vault, 'db_hashicorp_vault_engine': db_hashicorp_vault_engine, 'db_hashicorp_vault_secret_path': db_hashicorp_vault_secret_path, 'db_hashicorp_vault_username_key': db_hashicorp_vault_username_key, 'db_hashicorp_vault_secret_key': db_hashicorp_vault_secret_key, 'db_azure_vault_name': db_azure_vault_name, 'db_azure_vault_username_key': db_azure_vault_username_key, 'db_azure_vault_secret_key': db_azure_vault_secret_key, 'db_cyberark_vault_query_string': db_cyberark_vault_query_string, 'external_file_path': external_file_path, 'mount_base': mount_base, 'load_backup_path': load_backup_path, 'backup_server_name': backup_server_name, 'backup_host_user': backup_host_user, 'backup_host': backup_host, 'dump_credentials': dump_credentials, 'source_host_user': source_host_user, 'db_user': db_user, 'db_password': db_password, 'db_use_kerberos_authentication': db_use_kerberos_authentication, 'staging_repository': staging_repository, 'staging_host_user': staging_host_user, 'validated_sync_mode': validated_sync_mode, 'dump_history_file_enabled': dump_history_file_enabled, 'drop_and_recreate_devices': drop_and_recreate_devices, 'sync_strategy': sync_strategy, 'ase_backup_files': ase_backup_files, 'pre_validated_sync': pre_validated_sync, 'post_validated_sync': post_validated_sync}.items() if v is not None}
        ...
        if conf:
            return conf
        body = {k: v for k, v in {'db_vault_username': db_vault_username, 'db_vault': db_vault, 'db_hashicorp_vault_engine': db_hashicorp_vault_engine, 'db_hashicorp_vault_secret_path': db_hashicorp_vault_secret_path, 'db_hashicorp_vault_username_key': db_hashicorp_vault_username_key, 'db_hashicorp_vault_secret_key': db_hashicorp_vault_secret_key, 'db_azure_vault_name': db_azure_vault_name, 'db_azure_vault_username_key': db_azure_vault_username_key, 'db_azure_vault_secret_key': db_azure_vault_secret_key, 'db_cyberark_vault_query_string': db_cyberark_vault_query_string, 'name': name, 'description': description, 'staging_environment_id': staging_environment_id, 'staging_environment_user_ref': staging_environment_user_ref, 'staging_repository_id': staging_repository_id, 'source_environment_user_ref': source_environment_user_ref, 'sync_policy_id': sync_policy_id, 'retention_policy_id': retention_policy_id, 'load_backup_path': load_backup_path, 'dump_history_file_enabled': dump_history_file_enabled, 'validated_sync_enabled': validated_sync_enabled, 'logsync_enabled': logsync_enabled, 'source_of_production_backup': source_of_production_backup, 'backup_server_name': backup_server_name, 'external_file_path': external_file_path, 'backup_password': backup_password, 'backup_host': backup_host, 'backup_host_user': backup_host_user, 'hooks': hooks, 'db_username': db_username, 'db_password': db_password, 'db_use_kerberos_authentication': db_use_kerberos_authentication}.items() if v is not None}
        return await make_api_request('PATCH', endpoint, params=params, json_body=body if body else None)
...
async def snapshot_bookmark_tool(
    ...
    if action == 'search_snapshots':
        ...
        params = build_params()
        _ctx = {k: v for k, v in locals().items() if v is not None and not k.startswith('_')}
        conf = check_confirmation('GET', '/snapshots/find_by_location', action, 'snapshot_bookmark_tool', confirmed or False, context=_ctx)
        if conf:
            return conf
        ...
        params = build_params()
        _ctx = {k: v for k, v in locals().items() if v is not None and not k.startswith('_')}
        conf = check_confirmation('GET', '/snapshots/find_by_timestamp', action, 'snapshot_bookmark_tool', confirmed or False, context=_ctx)
        if conf:
            return conf
