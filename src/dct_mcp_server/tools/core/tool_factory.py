...
def _create_tool_function(
    api_path: str, method: str, action: str, operation: Dict[str, Any], spec: Dict[str, Any]
) -> Tuple[Callable, str]:
    """
...
def _create_grouped_tool_function(
    tool_name: str, description: str, apis: List[Dict[str, str]], spec: Optional[Dict[str, Any]]
) -> Tuple[Callable, str]:
    """
...
"""
    ...
    async def grouped_tool(action: str, **kwargs):
        ...
        return await _dct_client.make_request(
            method, final_path, params=query_params if query_params else None, json=json_body
        )
