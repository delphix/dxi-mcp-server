...
Verifies the two-step destructive-op contract for delete_vdb (manual
confirmation level per config/mappings/manual_confirmation.txt):
...
@pytest.mark.asyncio
async def test_delete_vdb_two_step_confirmation(mcp_client_self_service, dct_stub):
    vdb_id = "v-1"
    ...
    assert not dct_stub.received_request("POST", f"/dct/v3/vdbs/{vdb_id}/delete"), (
        "MCP server must NOT have sent the delete request before confirmation"
    )

    # --- Step 2 — second call with confirmed=True actually issues the DELETE ---
    second = await mcp_client_self_service.call_tool(
        "data_tool",
        {"action": "delete_vdb", "vdb_id": vdb_id, "confirmed": True},
    )
    assert not second.is_error, f"Confirmed call should succeed: {second}"
    assert dct_stub.received_request("POST", f"/dct/v3/vdbs/{vdb_id}/delete"), (
        "MCP server must have sent the delete request after confirmation"
    )
