...
async def test_dsource_workflow(mcp_client_self_service, dct_stub):
    ...
    res = await client.call_tool("data_tool", {"action": "search_dsources", "limit": 10})
    ...
    res = await client.call_tool("data_tool", {"action": "get_dsource", "dsource_id": ds_id})
    ...
    res = await client.call_tool("data_tool", {"action": "get_dsource_tags", "dsource_id": ds_id})
    assert not res.is_error
