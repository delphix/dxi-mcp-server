...
async def test_dsource_workflow(mcp_client_self_service, dct_stub):
    ...
    res = await client.call_tool("dsource_tool", {"action": "search", "limit": 10})
    ...
    res = await client.call_tool("dsource_tool", {"action": "get", "dsource_id": ds_id})
    ...
    res = await client.call_tool(
        "dsource_tool", {"action": "list_snapshots", "dsource_id": ds_id}
    )
    assert not res.is_error
    assert dct_stub.received_request("GET", f"/dct/v3/dsources/{ds_id}/snapshots")

    # Prompt 38 — Get the tags for that dSource.
    res = await client.call_tool("dsource_tool", {"action": "get_tags", "dsource_id": ds_id})
    assert not res.is_error
