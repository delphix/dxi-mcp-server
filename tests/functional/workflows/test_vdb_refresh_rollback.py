...
Translates `.claude/test/testing/self_service.md` prompts 7-12:
    ...
    7.  Refresh that VDB by timestamp (one hour ago)
    8.  List snapshots, then refresh by the most recent snapshot
    9.  List bookmarks, then refresh from the first bookmark
...
refresh/rollback steps. Rollbacks are pre-confirmed (standard confirmation).
"""
...
async def test_vdb_refresh_rollback(mcp_client_self_service, dct_stub):
    ...
    res = await client.call_tool(
        "vdb_tool",
        {"action": "refresh_by_timestamp", "vdb_id": vdb_id,
    ...
    res = await client.call_tool(
        "vdb_tool", {"action": "list_snapshots", "vdb_id": vdb_id}
    )
    assert not res.is_error
    ...
    res = await client.call_tool(
        "vdb_tool",
        {"action": "refresh_by_snapshot", "vdb_id": vdb_id, "snapshot_id": snapshot_id},
    )
    assert not res.is_error
    ...
    res = await client.call_tool(
        "vdb_tool", {"action": "list_bookmarks", "vdb_id": vdb_id}
    )
    assert not res.is_error
    ...
    res = await client.call_tool(
        "vdb_tool",
        {"action": "refresh_from_bookmark", "vdb_id": vdb_id, "bookmark_id": bookmark_id},
    )
    assert not res.is_error
    ...
    res = await client.call_tool(
        "vdb_tool",
        {"action": "rollback_by_timestamp", "vdb_id": vdb_id,
    ...
    res = await client.call_tool(
        "vdb_tool",
        {"action": "rollback_by_snapshot", "vdb_id": vdb_id,
    ...
    res = await client.call_tool(
        "vdb_tool",
        {"action": "rollback_from_bookmark", "vdb_id": vdb_id,
