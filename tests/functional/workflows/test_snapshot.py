...
async def test_snapshot_workflow(mcp_client_self_service, dct_stub):
    ...
    res = await client.call_tool("snapshot_bookmark_tool", {"action": "search_snapshots", "limit": 10})
    ...
    res = await client.call_tool("snapshot_bookmark_tool", {"action": "get_snapshot", "snapshot_id": snap_id})
    ...
    res = await client.call_tool(
        "snapshot_bookmark_tool", {"action": "get_snapshot_timeflow_range", "snapshot_id": snap_id}
    )
    assert not res.is_error
    ...
    res = await client.call_tool(
        ...
        {"action": "add_snapshot_tags", "snapshot_id": snap_id,
    ...
    res = await client.call_tool(
        "snapshot_bookmark_tool",
        {"action": "delete_snapshot_tags", "snapshot_id": snap_id,
