...
mocked DCTAPIClient, await the (async) tool function directly, assert routing.
"""

import dct_mcp_server.tools.job_endpoints_tool as job_module


async def test_job_tool_search_routes_to_jobs_search_endpoint(mock_dct_client):
    """
    ...
    result = await job_module.job_tool(action="search", limit=10, sort="-start_time")
...
async def test_job_tool_get_requires_job_id(mock_dct_client):
    """
    ...
    result = await job_module.job_tool(action="get")
