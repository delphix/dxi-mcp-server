...
Set `DCT_TOOLSET=continuous_data_admin` in `.mcp.json` and restart the dct MCP server. Then execute the prompts top to bottom via the dct MCP tools — Claude is the runner. Prompts are chained: IDs discovered in earlier steps carry forward to later ones.
